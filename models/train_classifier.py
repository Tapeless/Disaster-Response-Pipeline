...
def load_data(database_filepath):
    '''
    load_data
    Loads data from SQL database and returns values to use in our model.

    Inputs:
    database_filepath: filepath to .db file generated from data/process_data.py

    Returns:
    X: Data to be input to our model - 1d array of strings
    y: Data to be input to our model - 2d array of int (0,1) of categories
    categories: list of the category names included in input database file.
    '''
...
def eval_model(Y_pred, Y_test, categories):
    ...
    for i in range(len(f1)):
        print(
            fmt.format(
                categories[i], np.round(
                    precision[i], 3), np.round(
                    recall[i], 3), np.round(
                        f1[i], 3)))

    # store results in dataframe
    results = pd.DataFrame(list(zip(categories, precision, recall, f1)), columns=[
